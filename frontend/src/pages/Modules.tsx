        ...
        <table className="w-full min-w-[640px] text-sm">
          ...
          <tbody>
            {!modules ? (
              ...
              modules.map((m, i) => (
                <tr key={m.id} className={`border-b border-grid last:border-0 ${m.isActive ? "" : "opacity-50"}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-block rounded-md px-2 py-0.5 text-xs font-medium ${
                      m.isActive ? "border border-good/30 text-good" : "border border-grid text-muted"
                    }`}>
